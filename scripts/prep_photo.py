...
def prep_photo(inp, out):
    if not os.path.isfile(inp):
        print(f"error: input image not found: {inp}", file=sys.stderr)
        sys.exit(1)
    ...
    # clipLimit controls how much contrast enhancement is applied per tile;
    # tileGridSize divides the image into 8x8 regions for local adaptation.
    gray = cv2.cvtColor(rgb, cv2.COLOR_RGB2GRAY)
    clahe = cv2.createCLAHE(clipLimit=2.6, tileGridSize=(8, 8))
    gray = clahe.apply(gray)

    # a touch of global lift so the face sits in the sparse end of the ramp
    # alpha=1.05 is a mild contrast boost, beta=18 lifts the overall brightness
    gray = cv2.convertScaleAbs(gray, alpha=1.05, beta=18)

    # 3. paste onto white using the alpha mask (feathered to avoid a halo)
    # GaussianBlur with ksize=(0,0) auto-computes kernel size from sigmaX=1.0
    mask = (alpha.astype(np.float32) / 255.0)
    mask = cv2.GaussianBlur(mask, (0, 0), 1.0)
    result = gray.astype(np.float32) * mask + 255.0 * (1.0 - mask)
    result = np.clip(result, 0, 255).astype(np.uint8)

    Image.fromarray(result, mode="L").save(out)
    print("wrote", out, result.shape)
